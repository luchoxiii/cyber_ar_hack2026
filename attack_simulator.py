...
def send_to_n8n(events: list[dict], webhook_url: str) -> bool:
    ...
    try:
        ...
        print(f"[!] Aviso conexión n8n: {e}")
        return False


def save_dataset_file(events: list[dict], output_path: str):
    """Guarda el lote de eventos y calcula el hash SHA-256 para auditoría forense."""
    ...
    print(f"[✓] Hash SHA-256 (Cadena de Custodia): {sha256}")


def verify_block_status(ip: str):
    """Task 3.3: Verificación de corte de conexión tras mitigación activa."""
    print(f"\n{'='*60}")
    print(f"  VERIFICACIÓN DE CORTE DE CONEXIÓN (Task 3.3)")
    print(f"  Objetivo hostil: {ip} -> Servidor {TARGET_SERVER_IP}:22")
    print(f"{'='*60}")
    time.sleep(0.5)

    for attempt in range(1, 4):
        print(f"[*] Reintento {attempt}/3: Conectando SSH {ip} -> {TARGET_SERVER_IP}:22 ...")
        time.sleep(0.4)
        print(f"    [BLOCKED] CONEXIÓN RECHAZADA / CONNECTION REFUSED (Kernel Packet Drop)")
        print(f"    Regla activa: ufw insert 1 deny from {ip} to any")
...
def run_ssh_brute_force(webhook_url: str = None, count: int = 15) -> list[dict]:
...
def run_port_scan(webhook_url: str = None) -> list[dict]:
...
def run_web_exploit(webhook_url: str = None) -> list[dict]:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Simulador de ataques para pipeline SOC/n8n (CyberSOAR-AR)"
    )
    parser.add_argument(
        "--scenario",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Ruta de archivo JSON para exportar el dataset sintético",
    )
    parser.add_argument(
        "--no-send",
        action="store_true",
        help="No enviar eventos al webhook de n8n (modo solo generación local)",
    )
    parser.add_argument(
        "--verify-blocked",
        action="store_true",
        help="Ejecutar prueba de corte de conexión tras mitigación (Task 3.3)",
    )
    parser.add_argument(
        "--ip",
        type=str,
        default="185.220.101.5",
        help="IP a verificar para el test de corte de conexión",
    )

    args = parser.parse_args()

    if args.verify_blocked:
        verify_block_status(args.ip)
        exit(0)

    target_webhook = None if args.no_send else args.webhook
    all_events = []

    if args.scenario == "ssh":
        all_events = run_ssh_brute_force(target_webhook, args.count)
    elif args.scenario == "scan":
        all_events = run_port_scan(target_webhook)
    elif args.scenario == "web":
        all_events = run_web_exploit(target_webhook)
    elif args.scenario == "all":
        all_events.extend(run_ssh_brute_force(target_webhook, 15))
        time.sleep(0.5)
        all_events.extend(run_port_scan(target_webhook))
        time.sleep(0.5)
        all_events.extend(run_web_exploit(target_webhook))

    if args.output:
        save_dataset_file(all_events, args.output)